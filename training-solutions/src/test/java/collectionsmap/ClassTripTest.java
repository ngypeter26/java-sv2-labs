...
class ClassTripTest {
    ...
    @Test
    void testLoadInpaymentsWrongName() {
        Exception ex = assertThrows(IllegalArgumentException.class, () -> classTrip.loadInpayments(Paths.get("src/test/resources/inpaymentsWithWrongName.txt")));
        assertEquals("Empty name in line:    : 5000", ex.getMessage());
    }
    @Test
    void testLoadInpaymentsWrongPayment() {
        Exception ex = assertThrows(IllegalArgumentException.class, () -> classTrip.loadInpayments(Paths.get("src/test/resources/inpaymentsWithWrongPayment.txt")));
        assertEquals("The payment is invalid in line: Nagy Béla: 50x00", ex.getMessage());
    }
}